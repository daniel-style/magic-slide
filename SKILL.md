...
**NON-NEGOTIABLE QA GATE:** After `serve.py` is running for a newly generated deck, open `?ms_qa=overview&ms_qa_capture=1`, wait until the QA wall reports iframe-loaded readiness, capture one full-page/scrolling QA overview longshot, and repair the most obvious rendered visual issues by slide number first. Capture full-size single-slide screenshots only for problematic, ambiguous, marked, or high-risk slides; do not capture every slide individually by default. Follow `references/workflows/step-10-preview.md` for the detailed triage order. Then reopen/leave QA Overview running and stop: tell the user to mark slide changes with `Revise slide` on the QA cards and return when ready. This user revision pause is mandatory before final QA or delivery. When continuing repairs, first read unresolved JSON notes from `{topic}/sources/qa/visual-issues.json`.
...
4. **Generate once, inspect, then pause** — Produce the full deck after outline and Brief Lite, repair obvious QA overview longshot issues, then stop for the mandatory user `Revise slide` marking pass before final delivery.
