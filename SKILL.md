...
8. **Step 8: Preview, autonomous QA repair, mandatory user revision pause, revision-note repair, targeted full-size QA, and delivery** — ALWAYS launch the skill preview server with `scripts/serve.py`, open the runtime QA overview, repair obvious screenshot issues first, then stop so the user can mark slide revisions before final QA and delivery
...
**NON-NEGOTIABLE QA GATE:** After `serve.py` is running for a newly generated deck, open `?ms_qa=overview` or press `Q`, scan every QA card, capture scrolling QA overview screenshots first, and repair the most obvious rendered issues first. Capture full-size single-slide screenshots only for problematic, ambiguous, marked, or high-risk slides; follow `references/workflows/step-10-preview.md` for the detailed triage order. Then reopen/leave QA Overview running and stop: tell the user to mark slide changes with `Revise slide` on the QA cards and return when ready. This user revision pause is mandatory before final QA or delivery. When continuing repairs, first read unresolved JSON notes from `{topic}/sources/qa/visual-issues.json`.
...
4. **Generate once, inspect, then pause** — Produce the full deck after outline and Brief Lite, repair obvious QA overview screenshot issues, then stop for the mandatory user `Revise slide` marking pass before final delivery.
